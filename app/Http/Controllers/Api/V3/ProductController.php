...
class ProductController extends BaseController
{
    ...
    {
        $products['data'] = Product::latest()->get();
    ...
    public function show($id)
    {
        ...
        if ($product['data']) {
            $product['data'] = new ProductDetailResource($product['data']);
        }
    ...
    {
        $products['data'] = Product::where('added_by', 'admin')->latest()->get();
        if ($products['data']->count() > 0) {
            $products['data'] = ProductResource::collection($products['data']);
        }
    ...
    {
        $products['data'] = Product::where('added_by', 'seller')->latest()->get();
        if ($products['data']->count() > 0) {
            $products['data'] = ProductResource::collection($products['data']);
        }
    ...
    public function category($id)
    {
        ...
        $category_ids[] = $id;
        $cat['data'] = ProductResource::collection(Product::whereIn('category_id', $category_ids)->latest()->get());
    ...
    public function subCategory($id)
    {
        ...
        if ($category_ids) {
            $category_ids[] = $id;
            $products = Product::whereIn('category_id', $category_ids)->latest()->get();
            if ($products->count() > 0) {
    ...
    {
        $products['data'] = Product::where('brand_id', $id)->latest()->get();
        if ($products['data']->count() > 0) {
    ...
    {
        $products['data'] = Product::where('todays_deal', '1')->latest()->get();
        if ($products['data']->count() > 0) {
            $products['data'] = ProductResource::collection($products['data']);
        }
    ...
    {
        $fetured['data'] = ProductResource::collection(Product::where('featured', 1)->latest()->get());
    ...
    public function related($id)
    {
        ...
        $related['data'] = ProductResource::collection(Product::where('category_id', $product->category_id)->where('id', '!=', $id)->limit(10)->get());
    ...
    {
        $product = Product::find($id);
        $related['data'] = ProductResource::collection(Product::where('user_id', $product->user_id)->orderBy('num_of_sale', 'desc')->limit(4)->get());
    ...
    public function search(Request $request)
    {
        ...
        $key = $request->key;
        $scope = $request->scope;
        $products = Product::where('name', 'like', "%{$key}%")->orWhere('name_ar', 'like', "%{$key}%")->orWhere('tags', 'like', "%{$key}%");
        if ($products->count() > 0) {
            switch ($scope) {
