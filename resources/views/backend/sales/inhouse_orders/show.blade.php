    				...
    				<table class="table table-bordered invoice-summary">
        				<thead>
            				<tr class="bg-trans-dark">
                                <th class="min-col">#</th>
                                                         <th class="min-col">{{translate('Vendor')}}</th>
        				...
        				<tbody>
                            ...
                            @foreach ($order->orderDetails->where('seller_id', $admin_user_id) as $key => $orderDetail)
                                <tr>
                                    <td>{{ $key+1 }}</td>
                                       @php
                        $type = App\User::find( $orderDetail->product->user_id )->user_type;
                        @endphp
                        @if($type == 'admin' )
                                                <td>{{get_setting('website_name')}}</td>
                                                @else 
                                                <td>{{ App\User::find( $orderDetail->product->user_id )->name}}</td>
    			...
    			<table class="table">
          			<tbody>
                        ...
                        <tr>
            				...
            				<td>        					{{ single_price(App\BusinessSetting::where('type','shipping_cost')->first()->value) }}</td>
            			</tr>
                        <tr>
            				<td>
