                ...
                @if($seller->user != null && $seller->user->shop != null)
                    <tr>
                        ...
                        <td>{{$seller->user->email}}</td>
                         <td>@if($seller->paid == 1 ) {{translate('Paid')}} @else   {{translate('Unpaid')}}  @endif</td>
                        <td>
                        ...
                        <td>
                            <label class="aiz-switch aiz-switch-success mb-0">
                                <input onchange="update_verfiy(this)" value="{{ $seller->id }}" type="checkbox" <?php if($seller->verify == 1) echo "checked";?> >
...
@section('script')
    <script type="text/javascript">
        ...
        function update_verfiy(el){
            ...
            $.post('{{ route('sellers.verfy') }}', {_token:'{{ csrf_token() }}', id:el.value, status:status}, function(data){
                if(data == 1){
                    AIZ.plugins.notify('success', '{{ translate('رثبseller verified successfully') }}');
                }
                else{
